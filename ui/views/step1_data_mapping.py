...
def _render_working_copy_metrics(df: pd.DataFrame, file_bytes: bytes) -> None:
    ...
    c1.metric("Строк", _fmt_int(rows), delta=d_rows, delta_color="inverse")
    c2.metric("Колонок", df.shape[1])
    c3.metric("Дубликаты", _fmt_int(dups), delta=d_dups, delta_color="inverse")
    c4.metric("Средняя доля пропусков", miss_display, delta=d_miss, delta_color="inverse")
...
def page():
    render_page_header(
        "Данные и сопоставление колонок",
        "Загрузите CSV-файл, выберите тип данных и назначьте роли колонкам. "
        "После этого можно переходить к анализу и настройке признаков.",
        eyebrow="Шаг 1",
    )
    ...
    if state.contracts is None:
        try:
            ...
            page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Пока недоступно описание полей с сервера.")
            return
    ...
    with section_card(
        "Источник данных",
        "Загрузите файл и выберите сценарий, к которому относится таблица.",
    ):
        up_col, type_col = st.columns([1.3, 1], gap="large")

        with up_col:
            render_field_intro("CSV-файл", "Файл CSV с разделителем запятая, кодировка UTF-8.")
            up = st.file_uploader(
                "CSV-файл",
                type=["csv"],
                help="Файл CSV с разделителем запятая, кодировка UTF-8.",
        ...
        with type_col:
            ...
            render_field_intro("Тип данных", "От этого выбора зависит набор обязательных колонок.")
    ...
    if not state.working_file_bytes:
        ...
        page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Сначала загрузите CSV-файл.")
        return
    ...
    try:
        ...
        page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Исправьте файл, чтобы перейти к анализу.")
        return

    with section_card("Краткая диагностика", "Быстрый обзор структуры файла до сопоставления колонок."):
        _render_working_copy_metrics(df, state.working_file_bytes)

    with section_card(
        "Предпросмотр данных",
        "Проверьте названия колонок, формат значений и общую структуру таблицы.",
    ):
        st.dataframe(df.head(12), width="stretch", height=260)
        ...
        with st.expander("Все колонки: типы и примеры", expanded=False):
            ...
            st.dataframe(pd.DataFrame(meta_rows), width="stretch", height=220, hide_index=True)

    with section_card(
        "Очистка рабочей копии",
        "Изменения применяются только внутри текущего сеанса. Исходный файл не меняется.",
    ):
        d1, d2, d3 = st.columns(3)

        with d1:
            if st.button("Удалить дубликаты", type="secondary", use_container_width=True):
                ...
                if removed_dup == 0:
                    state.clean_notice_dedupe = "Полных дубликатов не найдено."
                else:
                    state.clean_notice_dedupe = (
                        f"Удалено дубликатов: {_fmt_int(removed_dup)} "
                        f"(строк-дубликатов было: {_fmt_int(before_dup)})."
                    )
                st.rerun()
        ...
        with d2:
            if st.button("Удалить строки с пропусками", type="secondary", use_container_width=True):
                ...
                if removed_na == 0:
                    state.clean_notice_dropna = "Строк с пропусками не найдено."
                else:
                    state.clean_notice_dropna = (
                        f"Удалено строк: {_fmt_int(removed_na)} "
                        f"({_fmt_int(before_rows)} → {_fmt_int(after_rows)})."
                    )
                st.rerun()
        ...
        with d3:
            if state.file_bytes and state.working_file_bytes != state.file_bytes:
                if st.button("Вернуть исходный файл", type="secondary", use_container_width=True):
                    ...
                    state.clean_notice_dedupe = None
                    state.clean_notice_dropna = None
                    state.clean_notice_restore = "Возвращён исходный файл."
                    st.session_state.pop("_metric_snapshot", None)
                    st.session_state.pop("_inspect_data_sig", None)
                    st.rerun()
        ...
        if state.did_dedupe or state.did_dropna:
            note = []
            if state.did_dedupe:
                note.append("дубликаты обработаны")
            if state.did_dropna:
                note.append("пропуски обработаны")
            st.caption(f"Текущая рабочая копия: {' · '.join(note)}")
    ...
    if state.inspect is None:
        with st.spinner("Проверяем файл на сервере…"):
            try:
                ...
                page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Сервер пока не смог проанализировать файл.")
                return
    ...
    with section_card(
        ...
        "Выберите, какая колонка в вашем файле соответствует каждой роли.",
    ):
        if state.template == "transactions":
            st.info(
                "Если в таблице нет готовой суммы операции, оставьте поле «Сумма операции» пустым "
                "и укажите «Цена за единицу» и «Количество». Сумма будет рассчитана автоматически."
            )
        ...
        with st.form("mapping_form_step1"):
            st.markdown(
                '<p class="ui-form-block-title">Обязательные поля</p>',
            ...
            for field, meta in required.items():
                ...
                render_field_intro(meta.get("title", field), label_text, mapping_compact=True)
                choice = st.selectbox(
                    f"Колонка для {meta.get('title', field)}",
                    options=options,
                    index=idx,
                    key=f"req_{field}",
                    label_visibility="collapsed",
            ...
            st.markdown(
                '<p class="ui-form-block-title">Необязательные поля</p>',
                unsafe_allow_html=True,
            )
    ...
    if not state.mapping:
        st.info("Сначала сохраните сопоставление колонок.")
        page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Сначала сохраните сопоставление колонок.")
        return

    missing_required = [f for f in required if not state.mapping.get(f)]
    if missing_required:
        st.error("Заполните обязательные поля: " + ", ".join(missing_required))
        page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Заполните обязательные поля перед переходом.")
        return

    if state.template == "transactions" and not transactions_financial_sources_ok(state.mapping):
        st.error(
            "Для покупок нужно указать либо «Сумма операции», либо обе колонки: "
            "«Цена за единицу» и «Количество»."
        )
        page_nav(None, STEP_2_ANALYTICS, next_disabled_reason="Укажите финансовые колонки для транзакций.")
        return
    ...
    with section_card(
        ...
        "Выберите полезные колонки для модели. Для категориальных признаков появятся рекомендации по обработке.",
    ):
        audits = []
        for c in extra_cols:
        ...
        if not audit_df.empty:
            ...
            st.dataframe(audit_df, width="stretch", hide_index=True, height=260)

        selectable = [x["Колонка"] for x in audits if x["Оценка"] in ("allow", "review")]
        chosen = st.multiselect(
            "Выберите дополнительные колонки",
            options=selectable,
        ...
        if chosen:
            ...
            for col in chosen:
                ...
                with st.container(border=True):
                    ...
                    st.caption(f"Примеры: {' | '.join(samples)}")

                    if profile["kind"] in {
                        "categorical_low_card",
                        "categorical_medium_card",
                        ...
                        selected_encoding = st.radio(
                            ...
                            options=options,
                            index=options.index(default_cfg.get("encoding"))
                            if default_cfg.get("encoding") in options
                            else 0,
                            format_func=lambda x: "One-hot encoding" if x == "onehot" else "Компактное кодирование",
                            horizontal=True,
                        ...
                        if selected_encoding == "onehot" and int(profile["nunique"]) > LOW_CARDINALITY_LIMIT:
                            st.warning(
                                ...
                                "Лучше использовать компактное кодирование."
                            )
                        ...
                        plot_category_profile(
                            ...
                            title=f"Распределение значений: `{col}`",
                        )
                    else:
                        st.success("Колонка будет агрегирована автоматически.")
                        state.extra_feature_config[col] = {
                            **default_cfg,
                            "encoding": None,
                            "top_values": profile["top_values"],
