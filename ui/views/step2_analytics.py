...
def page():
    ...
    st.caption(
        "Сначала проверьте, что файл читается корректно и содержит полезную историю по клиентам. "
    ...
    summary_metrics: list[tuple[str, str]] = [
        ...
        (f"Уникальных {_entity_label(state.template)}", _fmt_int(dfx[entity_src].nunique()) if entity_src and entity_src in dfx.columns else "—"),
    ...
    if time_src and entity_src:
        ...
        st.caption(
            "В этом блоке показаны агрегаты по времени и по клиентам/объектам. "
            "Их проще интерпретировать для диплома, чем распределения по отдельным строкам файла."
        )
    else:
    ...
    st.caption(
        ...
        "нет ли пропусков, перекосов по времени и слишком короткой истории по клиентам."
    )
    ...
    with q_right:
        if entity_src:
            ...
            st.caption(
                ...
                "Он нужен для понимания полноты наблюдений, а не для оценки выручки."
            )
        else:
    ...
    if time_src:
        ...
        st.caption(
            "График ниже показывает именно число строк в файле по времени. "
            "Он помогает заметить провалы загрузки или резкие изменения структуры данных."
        )
    else:
    ...
    with st.expander("Показать технические детали и дополнительные признаки", expanded=False):
        ...
        st.info(
            ...
            "в этой задаче реальный target формируется позже на snapshot-датасете, а не хранится как готовая колонка."
        )

    st.markdown("### Дальше")
    st.write("Если файл выглядит корректно и бизнес-графики читаются понятно, переходите к обучению модели.")
