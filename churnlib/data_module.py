...
from churnlib.feature_module import build_transaction_features, aggregate_extra_features
...
def canonicalize_types(df: pd.DataFrame, template: str) -> pd.DataFrame:
    ...
    if template == "transactions":
        ...
        if "is_cancellation" in df.columns:
            ...
            # дедфолт эвристика: транзакции, начинающиеся на "C"
...
def build_snapshots(df: pd.DataFrame, cfg: SnapshotConfig) -> pd.DataFrame:
    ...
    if template == "transactions":
        ...
        for t in anchors:
            ...
            feat["value_proxy"] = feat.get("monetary", 0.0)
        ...
        for t in anchors:
            ...
            future_active = set(future.loc[future["subscription_status"].isin(["active"]), "account_id"].unique())

            last_end = past.groupby("account_id")["period_end_dt"].max()
            mrr_sum = past.groupby("account_id")["mrr"].sum()
            mrr_last = past.sort_values("period_end_dt").groupby("account_id", as_index=True)["mrr"].last()
            ...
            if extra_cols:
                ...
                out = out.merge(extra_df, on="entity_id", how="left")
        ...
        for t in anchors:
            ...
            out = pd.DataFrame(base_rows)
            if extra_cols and not out.empty:
                extra_df = aggregate_extra_features(
                    past,
                    id_col="subject_id",
                    time_col="event_time",
...
def build_latest_snapshot(df: pd.DataFrame, cfg: SnapshotConfig) -> pd.DataFrame:
    """
    Для скоринга: берём последний anchor_time (tmax) и строим признаки только по past window.
    """
    template = cfg.template
    extra_cols = cfg.extra_feature_cols or []
    extra_cfg = cfg.extra_feature_config or {}
    ...
    if template == "subscriptions":
        df = df.dropna(subset=["account_id", "period_start", "period_end", "mrr", "subscription_status"]).copy()
        df["period_end_dt"] = pd.to_datetime(df["period_end"])
        t = df["period_end_dt"].max().normalize()

        past_start = t - pd.Timedelta(days=cfg.history_days)
        past = df[(df["period_end_dt"] > past_start) & (df["period_end_dt"] <= t)].copy()
        if past.empty:
            return pd.DataFrame()

        last_end = past.groupby("account_id")["period_end_dt"].max()
        mrr_sum = past.groupby("account_id")["mrr"].sum()
        mrr_last = past.sort_values("period_end_dt").groupby("account_id")["mrr"].last()

        rows = []
        for aid in past["account_id"].unique():
            recency = (t - last_end.get(aid)).days if pd.notna(last_end.get(aid, pd.NaT)) else np.nan
            rows.append(
                {
                    "entity_id": aid,
    ...
    if template == "events":
        ...
        rows = []
        for sid in past["subject_id"].unique():
            if int(freq.get(sid, 0)) < cfg.min_events_in_history:
                continue
            recency = (t - last_evt.get(sid)).days if pd.notna(last_evt.get(sid, pd.NaT)) else np.nan
            rows.append(
